...
function addLog(botId, level, message) {
    try {
        db.prepare("INSERT INTO logs (bot_id, level, message) VALUES (?, ?, ?)").run(botId, level, message)
        const maxLogs = 500
        db.prepare("DELETE FROM logs WHERE bot_id = ? AND id NOT IN (SELECT id FROM logs WHERE bot_id = ? ORDER BY id DESC LIMIT ?)").run(botId, botId, maxLogs)
    } catch {}
    if (io) io.to(`bot:${botId}`).emit("log", { botId, level, message, timestamp: new Date().toISOString() })
}

function incrementStat(botId, field) {
    const today = new Date().toISOString().slice(0, 10)
    db.prepare(`
        INSERT INTO stats (bot_id, date, ${field}) VALUES (?, ?, 1)
        ON CONFLICT(bot_id, date) DO UPDATE SET ${field} = ${field} + 1
    `).run(botId, today)
}

function updateBotStatus(botId, userId, status) {
...
async function startBot(botId, userId, phone) {
    ...
    updateBotStatus(botId, userId, "connecting")
    ...
    sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
        ...
        if (connection === "open") {
            updateBotStatus(botId, userId, "connected")
            const phoneNum = sock.user?.id?.split(":")[0] || phone
            if (phoneNum) db.prepare("UPDATE bots SET phone = ? WHERE id = ?").run(phoneNum, botId)
        ...
        if (connection === "close") {
            ...
            if (code === DisconnectReason.loggedOut) {
                ...
                updateBotStatus(botId, userId, "disconnected")
                delete instances[key]
            } else {
                addLog(botId, "warn", `🔄 Reconnexion... (code: ${code})`)
                updateBotStatus(botId, userId, "reconnecting")
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        for (const msg of messages) {
            ...
            const botConfig = db.prepare("SELECT * FROM bots WHERE id = ?").get(botId)
            ...
            const cmdRow = db.prepare("SELECT * FROM commands WHERE bot_id = ? AND name = ? AND enabled = 1").get(botId, cmdName)
