module.exports = async (sock, msg, from, args) => {
    ...
    if (!botParticipant || (botParticipant.admin !== "admin" && botParticipant.admin !== "superadmin")) {
        return sock.sendMessage(from, { text: "❌ Je ne suis pas administrateur dans ce groupe. Je ne peux pas bannir." })
    }
    ...
    // Sécurité
    if (target === botId) {
        return sock.sendMessage(from, { text: "❌ Je ne peux pas me bannir moi-même." })
    }
    ...
    try {
        ...
        await sock.sendMessage(from, {
            text: `✅ ${target.split("@")[0]} a été banni avec succès ! 🚫`
        }, { quoted: msg })
    } catch (err) {
        console.error(err)
        await sock.sendMessage(from, { text: "❌ Erreur lors du bannissement." })
    }
}

module.exports.description = "🚫 Bannit un membre du groupe (admin requis)"